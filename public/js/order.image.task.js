function asd(arr1, arr2) {
	...
	function init() {
	  ...
	  for ( var i=1; i<=arr1.length; i++ ) {
		...
		$('<div id="'+droppables[i-1]+'-drop">' + correct[i-1] + '</div>').text('').data( 'slot', correct[i-1] ).appendTo( '#droppablearea' ).droppable( {
		...
		*/
		$( '#'+droppables[i-1]+'-drop' ).css( 'background-image', 'url(/img/'+droppables[i-1]+'.gif) ' );
		$( '#'+droppables[i-1]+'-drop' ).css( 'background-repeat', 'no-repeat' );
		$( '#'+droppables[i-1]+'-drop' ).css( 'background-size', 'contain' );
		}
